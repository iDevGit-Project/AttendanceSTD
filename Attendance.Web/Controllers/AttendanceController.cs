...
namespace Attendance.Web.Controllers
{
    public class AttendanceController : Controller
    {
        ...
        //[HttpGet]
        //public IActionResult AttendanceReportByDate()
        //{
        //    return View(new AttendanceReportByDateVm());
        //}

        //[HttpPost]
        //public IActionResult AttendanceReportByDate(DateTime fromDate, DateTime toDate)
        //{
        //    var data = _db.AttendanceRecords
        //        .Include(x => x.Student)
        //        .Include(x => x.Session)
        //        .Where(x =>
        //            x.Session != null &&
        //            x.Session.CreatedAt >= fromDate &&
        //            x.Session.CreatedAt <= toDate
        //        )
        //        .Select(x => new AttendanceReportItemVm
        //        {
        //            StudentId = x.StudentId,
        //            FullName = x.Student!.FirstName + " " + x.Student.LastName,
        //            ImagePath = x.Student.PhotoPath,
        //            IsPresent = x.Status,
        //            DelayMinutes = x.LateMinutes
        //        })
        //        .ToList();
                                                                ...
                                                                [FromForm] int[]? studentIds)
                                                                    {
                                                                        ...
                                                                        if (!string.IsNullOrWhiteSpace(dateShamsi))
                                                                        {
                                                                            try
                                                                            {
                                                                                ...
                                                                                .Replace('۸', '8')
                                                                                .Replace('۹', '9');
                                                                                 sessionDate = PersianDateConverter.ParseShamsiToDate(dateShamsi).Date;
                                                                            }
                                                                            catch
                                                                            {
                                                                                return BadRequest(new { error = "تاریخ نامعتبر است" });
                                                                            }
                                                                        }
                                                                        else
                                                                        {
                                                                        ...
                                                                        var session = new AttendanceSession
                                                                        {
                                                                            ...
                                                                            Records = new List<AttendanceRecord>() // ✅ مهم
                                                                        };
                                                                        ...
                                                                        if (!string.IsNullOrWhiteSpace(startTime))
                                                                        {
                                                                            ...
                                                                            session.StartAt = sessionDate.Add(ts);
                                                                        }

                                                                        // ✅ grade فقط اگر واقعی است

...
namespace Attendance.Web.Controllers
{
    public class StudentsController : Controller
    {
        ...
        //    // 1) خواندن entities از DB (بدون فراخوانی توابع غیرقابل ترجمه داخل Select)
        //    var pageEntities = await query
        //        .Include(s => s.Class) // در صورت نیاز به نام کلاس در نگاشت
        ...
        //    // 2) نگاشت در سمت کلاینت و تبدیل تاریخ به شَمشـی
        ...
        // ---------------- Index: نمایش دانش‌آموزان با صفحه‌بندی و پشتیبانی AJAX ----------------
        public async Task<IActionResult> Index(int page = 1, int pageSize = 5, string? search = null)
        {
            if (page < 1) page = 1;
            var allowedPageSizes = new[] { 5, 10, 25, 50 };
        ...
        //    // === اصلاح شده: آمار کلی با نادیده‌گرفتن فیلترها (تا غیرفعال‌ها هم حساب شوند) ===
        //    var totalStudents = await _db.Students.IgnoreQueryFilters().CountAsync();
        //    var inactiveStudents = await _db.Students.IgnoreQueryFilters().CountAsync(s => !s.IsActive);

        //    // اگر خواستی تعداد فعال‌ها را هم صریح تنظیم کن (مطمئن باش مدل View آن را دارد)
        //    var activeStudents = totalStudents - inactiveStudents;
        ...
        //        TotalStudents = totalStudents,
        //        InactiveStudents = inactiveStudents,
        //        ActiveStudents = activeStudents, // اگر فیلد موجود است در ViewModel آن را مقداردهی کن
        //        Page = page,
        //        PageSize = pageSize,
        //        TotalPages = totalPages,
        //        Search = search
